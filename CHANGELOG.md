...
### Changed
- **The header readouts are arranged in three columns.** The two times stack on the left — when we come off, and what time it is now, the pair anybody reads together. The show's drift sits beside the projected end it explains. The two connection lamps hold the right-hand end, out of the way of the numbers until one of them goes red. Projected end comes first — it is the number people ask each other for, "what time are we off?" — with the show's drift beside it as the reason that number moves. The two connection lamps sit together at the far right, one above the other: they answer one question between them, and they are the only thing up there that is a state rather than a number, so the eye can skip them until one goes red.
