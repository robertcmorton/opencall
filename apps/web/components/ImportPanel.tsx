 ...
 */
/**
 * Kinds of show that are not offered when a sheet is first imported.
 *
 * Whether a fixture can go to extra time is a property of THAT MATCH, and it
 * is not usually known by whoever is loading the file — a junior or trial
 * game arrives on the same rugby league sheet as a premiership one. Offering
 * it here invites a guess at the wrong moment; it is chosen on the sheet
 * itself, where the person choosing is the person who knows.
 *
 * Everything else stays: the selector is how a sheet gets its rules, and it
 * grows as more sports are added.
 */
const NOT_AT_IMPORT = new Set(["nrl-no-extra"]);
              ...
              >
                ...
                {(["Sport", "Production"] as const).map((g) => (
                  <optgroup key={g} label={g}>
                    {EVENT_TYPES.filter((t) => t.group === g && !NOT_AT_IMPORT.has(t.id)).map((t) => (
